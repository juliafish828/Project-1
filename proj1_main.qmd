...
Next, we want to create two datasets, with one containing only non-county data, and the other containing only county data. We are able to do this based on how the area_name column is set up, and we also want to create new variables corresponding to either the county or state based on which dataset it is placed into.
...
class(state_tibble) <- c("county", class(state_tibble))
...
function1and2 <- function(messydata, col_name = )
```
...
## Function for step 4
```{r}
function4 <- function(long_data){
  subset_index <- grep(pattern = ", \\w\\w", long_data$area_name)
  state_tibble <- long_data[-subset_index, ]
  county_tibble <- long_data[subset_index, ]
  class(county_tibble) <- c("county", class(county_tibble))
  class(state_tibble) <- c("county", class(state_tibble))
...
## Putting it all into one function call